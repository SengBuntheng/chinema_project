...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($user_login_err) && empty($user_password_err)) {
    ...
    $sql = "SELECT user_id, username, password, user_type, status FROM tbl_login WHERE username = ? OR email = ?";
    ...
    if ($stmt = mysqli_prepare($link, $sql)) {
      ...
      if (mysqli_stmt_execute($stmt)) {
        ...
        if (mysqli_stmt_num_rows($stmt) == 1) {
          ...
          mysqli_stmt_bind_result($stmt, $user_id, $username, $hashed_password, $user_type, $status);
